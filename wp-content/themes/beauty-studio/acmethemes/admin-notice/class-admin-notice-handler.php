 ...
 *
 * Class Beauty_Studio_Notice_Handler
 */
class Beauty_Studio_Notice_Handler {
	...
	public function replace_term_ids( $replace_term_ids ){
		...
		/*Terms IDS*/
		$term_ids = array(
            'beauty-studio-newsnotice-cat',
            'beauty_studio_post_cat',
            'beauty_studio_post_tag',
            'beauty_studio_cat_id',
            'beauty_studio_wc_product_cat',
            'beauty_studio_wc_product_tag',
	...
	public function admin_notice() {
		...
		if ( ! $notice_nag ) {
			wp_enqueue_style( 'beauty-studio-notice', get_template_directory_uri() . '/acmethemes/admin-notice/admin-notice.css', array(), '3.0.0' );
			wp_enqueue_script( 'beauty-studio-adi-install', get_template_directory_uri()  . '/acmethemes/admin-notice/admin-notice.js', array( 'jquery' ), '', true );

			$translation = array(
			        'btn_text' => esc_html__( 'Processing...', 'beauty-studio' ),
                    'nonce'    => wp_create_nonce( 'beauty-studio-demo-import-nonce' ),
                    'adminurl'    => admin_url()
            );
			wp_localize_script( 'beauty-studio-adi-install', 'beauty_studio_adi_install', $translation );
	...
	public static function hide_notices() {
		...
		if ( isset( $_GET['at-gsm-hide-notice'] ) && isset( $_GET['at_gsm_admin_notice_nonce'] ) ) { // WPCS: input var ok.
			if ( ! wp_verify_nonce( wp_unslash( $_GET['at_gsm_admin_notice_nonce'] ), 'at_gsm_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.VIP.ValidatedSanitizedInput.InputNotSanitized
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'beauty-studio' ) ); // WPCS: xss ok.
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'beauty-studio' ) ); // WPCS: xss ok.
			}

			$notice_type = sanitize_text_field( wp_unslash( $_GET['at-gsm-hide-notice'] ) );
			update_option( 'beauty_studio_admin_notice_'. $notice_type, 1 );

			/*Update to Hide.*/
			if ( 'welcome' === $_GET['at-gsm-hide-notice'] ) {
				update_option( 'beauty_studio_admin_notice_'. $notice_type, 1 );
			} else { // Show.
				delete_option( 'beauty_studio_admin_notice_'. $notice_type );
			}
		}
	...
	public function getting_started() {
        ...
        <div id="at-gsm" class="updated notice-info at-gsm">
            <a class="at-gsm-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'activated' ), add_query_arg( 'at-gsm-hide-notice', 'welcome' ) ), 'at_gsm_hide_notices_nonce', 'at_gsm_admin_notice_nonce' ) ); ?>">
				<?php esc_html_e( 'Dismiss', 'beauty-studio' ); ?>
            </a>
            <div class="at-gsm-container">
                <img class="at-gsm-screenshot" src="<?php echo esc_url(get_template_directory_uri().'/screenshot.jpg' )?>" alt="<?php esc_attr_e( 'Beauty Studio', 'beauty-studio' ); ?>" />
                <div class="at-gsm-notice">
                    <h2>
						<?php
						printf(
						/* translators: 1: welcome page link starting html tag, 2: welcome page link ending html tag. */
							esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer, please make sure you visit our %2$swelcome page%3$s.', 'beauty-studio' ), '<strong>'. wp_get_theme()->get('Name'). '</strong>','<a href="' . esc_url( admin_url( 'themes.php?page=beauty-studio-info' ) ) . '">','</a>' );
						?>
                    </h2>

                    <p class="plugin-install-notice"><?php esc_html_e( 'Clicking the button below will install and activate the Advanced Import plugin.', 'beauty-studio' ); ?></p>

                    <a class="at-gsm-btn button button-primary button-hero" href="#" data-name="" data-slug="" aria-label="<?php esc_attr_e( 'Get started with Beauty Studio', 'beauty-studio' ); ?>">
		                <?php esc_html_e( 'Get started with Beauty Studio', 'beauty-studio' );?>
                    </a>
                </div>
            </div>
        </div>
	...
	public function install_advanced_import() {
		...
		check_ajax_referer( 'beauty-studio-demo-import-nonce', 'security' );
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'beauty-studio' );
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'beauty-studio' );
	...
	public function setup_review_notice() {
		...
		// Set the installed time in `beauty_studio_theme_installed_time` option table.
		$option = get_option('beauty_studio_theme_installed_time' );

		if ( ! $option ) {
			update_option('beauty_studio_theme_installed_time', time() );
		}
	...
	public function display_review_notice() {
		 ...
		 */
		if ( ( get_option('beauty_studio_theme_installed_time' ) > strtotime( '-15 day' ) ) || ( $ignored_notice_partially > strtotime( '-15 day' ) ) || ( $ignored_notice ) ) {
			return;
		}
		?>
		<div class="notice updated at-review-notice">
			<p>
				...
				/* Translators: %1$s current user display name. */
					esc_html__(
						'Howdy, %1$s! It seems that you have been using this theme for more than 15 days. We hope you are happy with everything that the theme has to offer. If you can spare a minute, please help us by leaving a 5-star review on WordPress.org.  By spreading the love, we can continue to develop new amazing features in the future, for free!', 'beauty-studio'
					),
			...
			<div class="links">
				<a href="https://wordpress.org/support/theme/beauty-studio/reviews/?filter=5#new-post" class="btn button-primary" target="_blank">
					<span class="dashicons dashicons-thumbs-up"></span>
					<span><?php esc_html_e( 'Sure', 'beauty-studio' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice_partially=0" class="btn at-danger-btn">
					<span class="dashicons dashicons-calendar"></span>
					<span><?php esc_html_e( 'Maybe later', 'beauty-studio' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice=0" class="btn at-success-btn">
					<span class="dashicons dashicons-smiley"></span>
					<span><?php esc_html_e( 'I already did', 'beauty-studio' ); ?></span>
				</a>

				<a href="<?php echo esc_url( 'https://wordpress.org/support/theme/beauty-studio/' ); ?>" class="btn at-default-btn" target="_blank">
					<span class="dashicons dashicons-edit"></span>
					<span><?php esc_html_e( 'Got theme support question?', 'beauty-studio' ); ?></span>
				</a>
			</div>
	...
	public function remove_review_notice() {
		...
		global $current_user;
		$user_id                  = $current_user->ID;
		$theme_installed_time     = get_option('beauty_studio_theme_installed_time' );
		$ignored_notice           = get_user_meta( $user_id, 'remove_theme_review_notice', true );
		$ignored_notice_partially = get_user_meta( $user_id, 'nag_remove_theme_review_notice_partially', true );

		// Delete options data.
		if ( $theme_installed_time ) {
			delete_option('beauty_studio_theme_installed_time' );
		}
	...
	public function review_notice_enqueue() {
		...
		wp_enqueue_style( 'beauty-studio-review-notice', get_template_directory_uri()  . '/acmethemes/admin-notice/admin-notice.css' );
 ...
 * @since    1.0.0
 */
function beauty_studio_notice_handler( ) {
	return Beauty_Studio_Notice_Handler::instance();
}
beauty_studio_notice_handler()->run();


/*For this theme only*/
if( !function_exists( 'beauty_studio_ai_update_image_size') ){
	function beauty_studio_ai_update_image_size(){
		/*Thumbnail Size*/
...
add_action( 'advanced_import_before_content_screen', 'beauty_studio_ai_update_image_size' );