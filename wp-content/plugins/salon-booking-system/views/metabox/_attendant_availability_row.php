        ...
        <div class="row col-xs-12 sln-select-specific-dates-calendar <?php echo isset($row['select_specific_dates']) && $row['select_specific_dates'] ? '' : 'hide'?>">
            <?php if($show_specific_dates && $plugin->getSettings()->isResourcesEnabled()):?>
                <div class="col-xs-12 col-md-6 form-group sln-select">
                    <label> <?php echo __('Select resource', 'salon-booking-system'); ?></label>
                    <div class="sln_attendant_day_resource closed-">
                        <select class="sln-select select2-hidden-accessible" data-palceholder="<?php _e('Select a resource', 'salon-booking-system')?>"
                            name="<?php echo $prefix; ?>[day_specific_resource]" id="<?php echo SLN_Form::makeID($prefix.'[day_specific_resource]');?>" tabindex="-1" aria-hidden="true">
                            <option value='0'><?php _e('Select a resource'); ?></option>
                            <?php 
                            foreach($plugin->getRepository(SLN_Plugin::POST_TYPE_RESOURCE)->getAll() as $resource): ?>
                                <option class="red" value="<?php echo $resource->getId()?>"
            ...
            if($show_specific_dates): ?>
                <div class="col-xs-12 col-md-6 form-group sln-select">
                    <label> <?php echo __('Select service', 'salon-booking-system'); ?></label>
                    <div class="sln_attendant_day_service closed-">
                        <select id="<?php echo SLN_Form::makeID($prefix. '[day_specific_service]'); ?>"
                            ...
                            data-palceholder="<?php _e('Select a service', 'salon-booking-system')?>"
                            name="<?php echo $prefix; ?>[day_specific_service]" tabindex="-1" aria-hidden="true">
                            <option value='0' class="red"><?php _e('Select a service'); ?></option>
                            <?php 
                            $services = array();
                            ...
                            foreach($services as $service):
                                ...
                                $has_resource = false; ?>
                                <option class="red" value="<?php echo $service->getId()?>"
                                    data-resources="<?php foreach($resources as $resource){echo $resource->getId().','; $has_resource |= isset($row['day_specific_resource']) && $row['day_specific_resource'] == $resource->getId();} ?>"
