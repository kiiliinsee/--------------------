...
function sln_init($) {
    ...
    if ($("#salon-step-date").length) {
        ...
        $('[data-salon-toggle="next"]').on("click", function(e) {
            ...
            if ($("#sln-salon input.sln-invalid").parent().has(".sln-invalid__file")) {
                $("#sln-salon input:invalid").parent().find(".sln-invalid__file").remove()
            }

            if ($(form).has(".sln-input-file").length > 0) {
                if ($('.sln-input-file input[type="file"]').data("required")) {
                    let filesAmount = $(form).find(".sln-input-file .sln-input-file--list li").length - 1;

                    if (filesAmount > 0) {
                        $('.sln-input-file input[type="file"]').removeAttr("required");
                    } else {
                        $('.sln-input-file input[type="file"]').attr("required");
                    }
                }
            }
            ...
            if (form[0].checkValidity()) {
                ...
                if ($("#sln-salon input:invalid").attr('type') == 'file') {
                    $(
                        "#sln-salon input:invalid,#sln-salon textarea:invalid,#sln-salon select:invalid"
                    ).parent()
                     .prepend("<p class='sln-invalid__file'> " + salon.checkout_field_placeholder + "</p>")
                }
                $(
                    "#sln-salon input:invalid,#sln-salon textarea:invalid,#sln-salon select:invalid"
                )
