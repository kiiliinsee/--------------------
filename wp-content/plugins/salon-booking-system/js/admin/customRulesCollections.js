...
function sln_initSelectSpecificService(e){
    jQuery('.sln-select select').not('#_sln_attendant_availabilities___new___day_specific_service, #_sln_attendant_availabilities___new___day_specific_resource')
    .each(function(){
        jQuery(this).parent().find('span.select2.select2-container.select2-container--sln').remove()
    }).select2({
        containerCssClass:
            "sln-select-rendered " +
    ...
    jQuery('.sln_attendant_day_service select').not('#_sln_attendant_availabilities___new___day_specific_service').on('select2:open', function(){
        let available_services = jQuery('#_sln_attendant_services').val();
        available_services = available_services.map((val) => val.split('_').pop());
        let current_resource = Number(jQuery(this).closest('.row.sln-select-specific-dates-calendar').find('.sln_attendant_day_resource select').val());
        let has_curr_res = undefined;
        let li_el = null;
        jQuery(this).find('option').not('option[value="0"]').each(function(index, val){
            has_curr_res = jQuery(val).data('resources').split(',').includes(String(current_resource));
            console.log((available_services.length && available_services.includes(val.value) && current_resource && has_curr_res)
            , (available_services.length && !current_resource && available_services.includes(val.value))
            , (!available_services.length && current_resource && has_curr_res)
            , (!available_services.length && !current_resource))
            if(
                (available_services.length && available_services.includes(val.value) && current_resource && has_curr_res)
                || (available_services.length && !current_resource && available_services.includes(val.value))
                || (!available_services.length && current_resource && has_curr_res)
                || (!available_services.length && !current_resource)
            ){
                val.removeAttribute('disabled');
                setTimeout(function(value, li_el){
                    li_el = jQuery('#select2-_sln_attendant_availabilities_1_day_specific_service-results li').filter(function(){
                        return jQuery(this).attr('id') && value.value == jQuery(this).attr('id').split('-').pop();
                    });
                    li_el = li_el.length ? jQuery(li_el).removeAttr('style').removeAttr('aria-disabled').attr('aria-selected', false): li_el;
                }, 1, val, li_el)
                ...
                setTimeout(function(value, li_el){
                    ...
                    li_el = li_el.length ? jQuery(li_el).css('display', 'none').attr('aria-disabled', true).removeAttr('aria-selected') : li_el;
                }, 1, val, li_el);
