...
class SLB_Discount_Plugin {
	...
	public function hook_init() {
		...
		add_filter('sln.booking.getTotal', array($this, 'hook_booking_getTotal'), 10, 2);
...
//		}
		...
		$first = true;
		foreach ($discounts as $discountId) {
			if($discountId instanceof SLB_Discount_Wrapper_Discount){
				$discount = $discountId;
...
//				else {
                if ($item['price']) {
                    ...
                    if ($bookingService = $bookingServices->findByService($sId)) {
                        $service = $bookingService->getService();
                        if ($service->getVariablePriceEnabled() && $service->getVariablePrice($atId) !== '') {
                            $price = $service->getVariablePrice($atId);
                        } else {
                            $price = $service->getPrice();
                        }
                    } else {
...
//                    $price      = $item['price'] ?? ($bookingServices->findByService($sId) ?
//                        ($bookingServices->findByService($sId)->getVariablePriceEnabled() && $bookingServices->findByService($sId)->getVariablePrice($atId) !== '' ? $bookingServices->findByService($sId)->getVariablePrice($atId) : $bookingServices->findByService($sId)->getPrice())
//                        : '');
//                    $count     = isset($countServices[$sId]) ? $countServices[$sId] : 1;
//                    $price      = $price * $count;
//                }
	 ...
	 * @return float
	 */
	public function hook_booking_getTotal($total, $bb) {
		$discountData = $bb->getMeta('discount_amount');
	...
	public function hook_summary_before_total_amount($bb, $size) {
		...
		$discount     = false;
		$discountData = $bb->getMeta('discounts');
		if (!empty($discountData)) {
			$discountId = reset($discountData);
			$discount   = $this->createDiscount($discountId);
		...
		if ($discount) {
			...
			$pt = $bb->getPostType();
			delete_post_meta($bb->getId(), "_{$pt}_discount_amount");
			delete_post_meta($bb->getId(), "_{$pt}_discounts");
			delete_post_meta($bb->getId(), "_{$pt}_discount_score");
			$discountValue = 0.0;
		}
	...
	public function hook_history_nav(){
		echo '<li class="sln-account__nav__item sln-account__nav__discounts" role="presentation"><a data-target="#sln-account__discount__content" aria-controls="sln-account__discount__content" role="tab" data-toggle="tab"><span>'. __('Discounts', 'salon-booking-system').'</span></a></li>';
	}
