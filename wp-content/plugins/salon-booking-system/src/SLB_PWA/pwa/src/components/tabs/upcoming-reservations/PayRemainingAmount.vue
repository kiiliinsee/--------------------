<template>
    <span class="remaining-amount-payment-link" v-show="show">
        ...
        <b-alert :show="isSuccess" fade variant="success">{{ $t('successMessagePayRemainingAmount') }}</b-alert>
        <b-alert :show="isError" fade variant="danger">{{ $t('errorMessagePayRemainingAmount') }}</b-alert>
    </span>
</template>
