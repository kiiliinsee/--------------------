...
class SLN_Action_Ajax_Calendar extends SLN_Action_Ajax_Abstract
{
  ...
  public function execute()
  {
    ...
    $this->isYearly = date('m', strtotime($this->from)) == '01' && date('m', strtotime($this->to)) == '12';
    $this->buildBookings();
  ...
  private function getAssistants()
  {
    ...
    $to = Date::create($this->to)->getNextDate();
    foreach ($this->assistants as $att) {
      $availableTimes = array();
      if ($this->to->diff($this->from)->d == 6) {
        $day = Date::create($this->from);
        do {
          if ($att->getAvailabilityItems()->isValidDate($day) && $att->getNewHolidayItems()->isValidDate($day) && false) {
            $availableTimes[] = $this->getAjaxDayWeekStart($day);
          }
          $day = $day->getNextDate();
        } while (!$day->isEq($to));
      } else {
  ...
  private function wrapBooking($booking)
  {
    ...
    if ($this->isYearly) {
      ...
      $ret = array(
        ...
        "calendar_day_assistants_common" => mb_convert_encoding($this->getCalendarDayAssistantsCommon($booking), 'UTF-8', 'UTF-8'),
