...
foreach ($items as $key => $label) {
	...
	static public function fieldFile($name, $value, $settings){?>
		...
		<input class="sln-input-file--input" id="<?php echo self::makeID($name); ?>" <?php echo ($settings['required'] ? 'required' : '') ?> data-required="<?php echo ($settings['required'] ? 'true' : 'false') ?>" type="file" name="<?php echo $name. '[]' ?>" multiple accept="<?php echo is_array($settings['accept']) ? implode(',', $settings['accept']) : $settings['accept']; ?>">
